...
export class Profiler extends React.Component<ProfilerProps, ProfilerState> {
    ...
    private _renderMethodDropDown() {
        if (!addressUtils.isAddress(this.state.contractAddress)) {
            return null;
        }
        const methods = _.keys(API_PAYLOAD.gasCostByLineBySignature);
        const menuItems = _.map(methods, (method: string) => {
            return <MenuItem value={method} primaryText={method === '*' ? 'All methods' : method} />;
        });
        return (
    ...
    private _renderBars() {
        ...
        _.times(linesOfSource, () => {
            ...
            if (!_.isUndefined(logarithmicGasCostByLine[i])) {
                gasCost = API_PAYLOAD.gasCostByLineBySignature[this.state.selectedMethod][i];
    ...
    private _getLogGasCostByLine() {
        ...
        _.each(
            API_PAYLOAD.gasCostByLineBySignature[this.state.selectedMethod],
